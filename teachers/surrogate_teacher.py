...
def __select_example__(teacher, student, X, y, batch_size):
    ...
    for i in range(nb_batch):
        ...
        s -= lr * 2 * student.example_usefulness(teacher, data, label)
...
class SurrogateDiffLinearTeacher(SurrogateLinearTeacher):
    ...
    def __init__(self, feature_space, used_feature_space, normal_dist=True):
        ...
        if normal_dist:
            self.proj_mat = th.randn(feature_space, used_feature_space).cuda()
        else:
            self.proj_mat = th.rand(feature_space, used_feature_space).cuda()
