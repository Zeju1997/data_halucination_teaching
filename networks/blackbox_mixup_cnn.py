...
class UnrolledBlackBoxOptimizer(nn.Module):
    ...
    def forward(self, weight, w_star=None, w_init=None):
        ...
        for i in range(self.opt.n_unroll_blocks):
            ...
            # with torch.no_grad():
            #    for p, g in zip(self.student.parameters(), grad_stu):
            #        p.grad = g
            ...
            with torch.no_grad():
                for param, grad in zip(self.student.parameters(), grad_stu):
                    new_param = param.data - 0.001 * grad
                    param.data = new_param
                    # new_val = param.data - 0.1 * grad
                    # param.copy_(new_val)

            # w_g1 = []
            # for param in self.student.parameters():
            #     w_g1.append(param.data.clone())

            # mixed_x = alpha * gt_x_1 + (1 - alpha) * gt_x_2

            # outputs = self.student(mixed_x)
            # train_loss = alpha * self.loss_fn(outputs, gt_y_1) + (1 - alpha) * self.loss_fn(outputs, gt_y_2)
            # train_loss = train_loss.to(torch.float32)

            # loss_stu = loss_stu + train_loss # .clone()
            loss_stu = loss_stu + loss # .clone()
