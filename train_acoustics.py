...
class Trainer:
    ...
    def main(self):
        ...
        logname = os.path.join(self.opt.log_path, 'results' + '_' + 'ResNet50' + '_' + self.device + '_' + str(self.opt.seed) + '.csv')
        ...
        for epoch in tqdm(range(self.opt.n_epochs)):
            if epoch != 0:
                self.train(model, train_loader, self.loss_fn, optimizer, epoch)

            acc, test_loss = self.test(model, test_loader=test_loader, epoch=epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', type=str, default='acoustics_classifier',
                        help='For Saving the current Model')
    parser.add_argument('--data_dir', type=str, default='./data/acoustics',
                        help='For Saving the current Model')
